      ...
      <main className={`dz-platform-main${collapsed ? " dz-main-expanded" : ""}`}>
        ...
        {view === "table" ? (
          <div className="dz-card" style={{ padding: 0, overflow: "hidden" }}>
            <DraggablePatientTable patients={filtered} onRowClick={(id) => router.push(`/doczoc/patients/${id}`)} externalFocusMode={tableFocusMode} />
          </div>
        ) : (
          <div className="dz-patients-grid">
            {filtered.map((p) => (
              <Link href={`/doczoc/patients/${p.id}`} key={p.id} className="dz-patient-card">
